...
byte must follow the last pixel, in order for the final LED to latch.
...
values to the strip.  Curiously, zero bytes can only travel half a meter (16
LEDs) down the line before needing backup; the next half meter requires an
...
LPD8806RGBW::LPD8806RGBW(uint16_t n) {
  pixels = NULL;
  begun  = false;
  updateLength(n);
  updatePins();
...
LPD8806RGBW::LPD8806RGBW(uint16_t n, uint8_t dpin, uint8_t cpin) {
  pixels = NULL;
  begun  = false;
  updateLength(n);
...
void LPD8806RGBW::updatePins(void) {
...
void LPD8806RGBW::updatePins(uint8_t dpin, uint8_t cpin) {
...
void LPD8806RGBW::startSPI(void) {
#ifdef __AVR_ATtiny85__
  ...
  // SPI bus is run at 2MHz.  Although the LPD8806RGBW should, in theory,
...
uint16_t LPD8806RGBW::numPixels(void) {
  return numLEDs;
}
